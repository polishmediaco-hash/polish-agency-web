# Graph Report - polishmedia  (2026-09-10)

## Corpus Check
- 82 files · ~539,038 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 898 nodes · 1422 edges · 75 communities (68 shown, 5 thin omitted)
...
1. `triggerAutoSave()` - 41 edges
2. `pushHistory()` - 32 edges
3. `renderElement()` - 28 edges
4. `pushHistory()` - 19 edges
5. `bindKeyboardShortcuts()` - 16 edges
6. `updatePosition()` - 15 edges
7. `renderControls()` - 15 edges
8. `selectElement()` - 15 edges
9. `findElement()` - 14 edges
10. `renderPins()` - 13 edges
...
Cohesion: 0.10
Nodes (63): addCallout(), addConnection(), addDrawingStroke(), addForm(), addFormField(), addFrame(), addFrameBox(), addMetric() (+55 more)
...
Cohesion: 0.16
Nodes (12): { boardsService }, BUNDLED_BOARDS_DIR, express, fs, getStarterBoard(), path, readBoardFile(), { requireUserOrAdminAuth } (+4 more)
...
Cohesion: 0.14
Nodes (28): applyElementStyles(), attachElementInteractions(), createBeliefTriadDOM(), createBonusStackDOM(), createCadenceTimelineDOM(), createCalloutDOM(), createCapacityIndicatorDOM(), createDiagnosticProtocolDOM() (+20 more)
...
Cohesion: 0.12
Nodes (15): author, description, devDependencies, puppeteer, license, main, name, overrides (+7 more)
...
### Community 23 - "test-miro-suite.js"
Cohesion: 0.29
Nodes (6): puppeteer, fs, http, path, puppeteer, runMiroSuite()
...
Cohesion: 0.50
Nodes (4): scripts, dev, seed:supabase, start
...
### Community 31 - "POLISH Media Co — Project State & Handoff Document"
Cohesion: 0.08
Nodes (23): 1. Executive Summary & Brand Shift, 2. Core Pages & Component Architecture, 3. Backend Architecture (`server/index.js`), 4. Git & Repository Status, 5. Major Milestones & Integrations (September 2026 Session), 6. Next Priority Roadmap (Ready for Next Conversation), 7. How to Start a Fresh Antigravity Chat, A. Agent Reach & Internet Capabilities (`agent-reach`) (+15 more)
...
### Community 62 - "supabase.js"
Cohesion: 0.18
Nodes (6): cmsService, { createClient }, fs, keepAliveService, leadsService, path

### Community 63 - "api.js"
Cohesion: 0.18
Nodes (10): express, express, fs, getDefaultContent(), { leadsService, cmsService, keepAliveService }, { notifyNewLead, notifyNewMeeting, sendWhatsAppMessage }, path, readContent() (+2 more)
...
## Knowledge Gaps
- **323 isolated node(s):** `puppeteer`, `path`, `fs`, `AUDIT_VIEWPORTS`, `LANGUAGES` (+318 more)
...
- **Why does `puppeteer` connect `test-miro-suite.js` to `generate-social-avatars.js`, `package.json`, `capture-live-views.js`?**
